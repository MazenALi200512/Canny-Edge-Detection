...
static inline void scalar_pixel(const Image& input, int x, int y, int16_t& gx_out, int16_t& gy_out)
{
    int32_t gx = 0;
    int32_t gy = 0;
...
static void rvv_row_interior(const Image& input, Gradient& grad, int y)
{
    ...
    while(x < x_end)
    {
        ...
        for(int ky = -1; ky <= 1; ky++)
        {
            ...
            for(int kx = -1; kx <= 1; kx++)
            {
                ...
                gx_acc = __riscv_vadd_vv_i16m2(gx_acc, __riscv_vmul_vx_i16m2(px16, SOBEL_X[ky + 1][kx + 1], vl), vl);
                gy_acc = __riscv_vadd_vv_i16m2(gy_acc, __riscv_vmul_vx_i16m2(px16, SOBEL_Y[ky + 1][kx + 1], vl), vl);
            }
        }

        int idx = y * W + x;
        __riscv_vse16_v_i16m2(grad.gx.data() + idx, gx_acc, vl);
        __riscv_vse16_v_i16m2(grad.gy.data() + idx, gy_acc, vl);
        x += (int)vl;
    }
}
...
Gradient sobel_rvv(const Image& input)
{
    ...
    for(int y = 0; y < H; y++)
    {
        if(y == 0 || y == H - 1)
        {
            for(int x = 0; x < W; x++)
            {
                scalar_pixel(input, x, y, grad.gx[y * W + x], grad.gy[y * W + x]);
            }
            continue;
        }
        scalar_pixel(input, 0, y, grad.gx[y * W], grad.gy[y * W]);
        rvv_row_interior(input, grad, y);
        scalar_pixel(input, W - 1, y, grad.gx[y * W + (W - 1)], grad.gy[y * W + (W - 1)]);
    }
